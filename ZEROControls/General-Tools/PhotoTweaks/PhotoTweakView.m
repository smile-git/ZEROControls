...
- (instancetype)initWithCornerType:(CropCornerType)cornerType {
...
- (void)updateLines:(NSArray *)lines horizontal:(BOOL)horizontal {
    ...
    [lines enumerateObjectsUsingBlock:^(UIView *_Nonnull line, NSUInteger idx, BOOL * _Nonnull stop) {
        
        if (horizontal) {
            line.frame = CGRectMake(0, (self.frame.size.height / (lines.count + 1)) * (idx + 1), self.frame.size.width, 1 / [UIScreen mainScreen].scale);
        } else {
            line.frame = CGRectMake((self.frame.size.width / (lines.count + 1)) * (idx + 1), 0, 1 / [UIScreen mainScreen].scale, self.frame.size.height);
        }
    }];
}

#pragma mark - Dismiss Lines
...
@property (nonatomic, assign) CGFloat cropViewCenterY;
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        // ----- scale the image
        _maximumCanvasSize = CGSizeMake(kMaximumCanvasWidthRatio * self.frame.size.width, kMaximumCanvasHeightRatio *self.frame.size.height - kCanvasHeaderHeigth);
        ...
        _cropViewCenterY = self.maximumCanvasSize.height / 2 + kCanvasHeaderHeigth;
        
        _scrollView = [[PhotoScrollView alloc] initWithFrame:bounds];
        _scrollView.center = CGPointMake(CGRectGetWidth(self.frame) / 2, self.cropViewCenterY);
        _scrollView.bounces = YES;
        _scrollView.layer.anchorPoint = CGPointMake(0.5, 0.5); // ----- 设置默认支点
        ...
        [_resetBtn setTitle:NSLocalizedStringFromTable(@"REST", @"PhotoTweaks", nil) forState:UIControlStateNormal];
...
- (void)cropEnded:(CropView *)cropView {
    ...
    // ----- calculate the new bounds of crop view - 计算裁剪视图的新bounds
    CGRect newCropBounds = CGRectMake(0, 0, scale * cropView.frame.size.width, scale * cropView.frame.size.height);
    
    // ----- calculate the new bounds of scroll view - 计算scroll view 的新bounds
    ...
    // ----- calculate the zoom area of scroll view - 计算scrollview缩放范围
    CGRect scaleFrame = cropView.frame;
    if (scaleFrame.size.width >= self.scrollView.bounds.size.height) {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        cropView.center = CGPointMake(CGRectGetWidth(self.frame) / 2, self.cropViewCenterY);
        
        // ----- zoom the specified area of scroll view - 缩放滚动视图到指定区域
...
- (void)resetBtnTapped:(id)sender {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.scrollView.center = CGPointMake(CGRectGetWidth(self.frame) / 2, self.cropViewCenterY);
...
- (CGPoint)photoTranslatoin {
    ...
    CGPoint zeroPoint = CGPointMake(CGRectGetWidth(self.frame) / 2, self.cropViewCenterY);
