...
open class FoldingCell: UITableViewCell {
    ...
    // UIView is displayed when cell close
    ...
    open var itemCount: NSInteger = 4
    
    // The color of the back cell
    open var backViewColor: UIColor = UIColor.brown
    ...
    func configureDefaultState() {
        ...
        // 当我们需要为某个视图指定固定宽度或者高度时，这时候secondItem为nil
    ...
    func createAnimationView() {
        ...
        // copy constraints from containerView  将self.contentView中与containerView相关的约束添加到animationView上
        ...
        for constraint in self.contentView.constraints {
            ...
            } else if let firstItem = constraint.firstItem as? UIView, let secondItem = constraint.secondItem as? UIView, secondItem == containerView {
                let newConstraint = NSLayoutConstraint(item: firstItem, attribute: constraint.firstAttribute, relatedBy: constraint.relation, toItem: animationView, attribute: constraint.secondAttribute, multiplier: constraint.multiplier, constant: constraint.constant)
                newConstraints.append(newConstraint)
            }
        }
        self.contentView.addConstraints(newConstraints)
        
        // added height constraint 将containerView的高度约束添加到animationView
        for constraint in containerView.constraints {
            if constraint.firstAttribute == .height, let item = constraint.firstItem as? UIView, item == containerView {
    ...
    func addImageItemsToAnimationView() {
        ...
        // added other views
        let itemHeight = (containerViewSize.height - foregroundViewSize.height * 2) / CGFloat(itemCount - 2)
        ...
        if let animationView = self.animationView {
            ...
            for case let container as RotatedView in animationView.subviews.sorted(by: { $0.tag < $1.tag })
                where container.tag > 0 && container.tag < animationView.subviews.count {
                    previousView?.addBackView(container.bounds.size.height, color: backViewColor)
    ...
    func closeAnimation(_ completoin: (() -> Void)?) {
...
// Helper function inserted bu Swift 4.2 migrator.
